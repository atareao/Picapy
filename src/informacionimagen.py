...
class InformacionImagen(Gtk.Dialog):
    ...
    def __init__(self, parent, imagen):
        ...
        table.attach(self.button4, 2, 3, 3, 4,
                     xoptions=Gtk.AttachOptions.FILL,
                     yoptions=Gtk.AttachOptions.SHRINK)
        ...
        table.attach(self.entry7, 1, 2, 6, 7,
                     xoptions=Gtk.AttachOptions.EXPAND,
                     yoptions=Gtk.AttachOptions.SHRINK)

        self.button7 = Gtk.Button()
        self.button7.connect('clicked', self.on_button7_clicked)
        self.button7.set_image(Gtk.Image.new_from_stock(
            Gtk.STOCK_COPY, Gtk.IconSize.BUTTON))
        table.attach(self.button7, 2, 3, 6, 7,
                     xoptions=Gtk.AttachOptions.FILL,
                     yoptions=Gtk.AttachOptions.SHRINK)
        ...
        if imagen is not None:
            ...
            self.entry3.set_text('%sx%s' % (imagen.params['width'],
                                            imagen.params['height']))
            self.entry4.set_text(imagen.params['url'])
            self.entry5.set_text(imagen.thumbnails[0]['url'])
            label5.set_label('%s %sx%s:' % (_('Thumbnail'),
                                            imagen.thumbnails[0]['width'],
                                            imagen.thumbnails[0]['height']))
            self.entry6.set_text(imagen.thumbnails[1]['url'])
            label6.set_label('%s %sx%s:' % (_('Thumbnail'),
                                            imagen.thumbnails[1]['width'],
                                            imagen.thumbnails[1]['height']))
            self.entry7.set_text(imagen.thumbnails[2]['url'])
            label7.set_label('%s %sx%s:' % (_('Thumbnail'),
                                            imagen.thumbnails[2]['width'],
